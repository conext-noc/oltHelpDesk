...
from helpers.operations.portHandler import dictToZero
...
def portOperation(comm, command, quit, olt, action):
    ...
    portCount = []
    FSP = None
    vp_count = {'vp_ttl': 0, "vp_active_cnt": 0, "vp_deactive_cnt": 0, "vp_los_cnt": 0, "vp_off_cnt": 0}
    if action == "VP":
    ...
    while keep:
        ...
        totalDeactClients = 0
        log(
            "| {:^6} | {:^6} | {:^40} | {:^11} | {:^7} | {:^15} | {:^6} | {:^14} | {:^14} | {:^10} | {:^16} |".format(
                "f/s/p",
                "onu_id",
                "name",
                "state",
        ...
        for client in clients:
            ...
            PWR = client["pwr"]
            CT = f"{DATE} {TIME}"
            resp = "| {:^6} | {:^6} | {:^40} | {:^11} | {:^7} | {:^15} | {:^6} | {:^14} | {:^14} | {:^10} | {:^16} |".format(
            ...
            if action == "VP":
                ...
                if CF == "active":
                    vp_count['vp_active_cnt'] += 1
                    if STATUS == "offline":
                        if CAUSE == "LOSi/LOBi" or CAUSE == "LOS":
                            vp_count['vp_los_cnt'] += 1
                            CT = f"{DATE} {TIME}"
        ...
        log(f"""
En el puerto {FSP}:
El total de clientes en el puerto es           :   {vp_count['vp_ttl']}
El total de clientes activos es                :   {vp_count['vp_active_cnt']}
El total de clientes desactivados es           :   {vp_count['vp_deactive_cnt']}
El total de clientes activos en corte es       :   {vp_count['vp_los_cnt']}
El total de clientes activos apagados es       :   {vp_count['vp_off_cnt']}
